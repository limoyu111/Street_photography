...
exp_name = '2023-06-06T09-44-57'
ckpt_dir = exp_root + exp_name + '_CrossAttention/'
...
def main():
    ...
    for augment_types in [[0,0], [1,0], [0,1], [1,1]]:
        augment_types = [augment_types] * 4
        opt.n_samples  = len(augment_types)
        outdir = os.path.join(opt.outdir, os.path.basename(opt.ckpt), 
                                'indicator{}{}_seed{}'.format(augment_types[0][0], augment_types[0][1], opt.seed))
        ...
        start_code = None
        if opt.fixed_code:
            # start_code = torch.randn([1, opt.C, opt.H // opt.f, opt.W // opt.f], device=device).repeat(opt.n_samples, 1, 1, 1)
                ...
                with model.ema_scope():
                    for index, batch in enumerate(dataloader):
                        ...
                        if image_name not in  ['000001051688_GT', '000000000283_GT', '000000354418_GT', 
                                            ...
                                            '000000421757_GT', '000000148330_GT', '000000079346_GT']:
                            continue
                        start = time.time()
                        ...
                        samples_ddim, z_denoise_row = sampler.sample(S=opt.ddim_steps,
                                                        ...
                                                        verbose=False,
                                                        eta=opt.ddim_eta,
                                                        mask=test_model_kwargs['bg_mask'],
                                                        x_T=start_code,
                        ...
                        # norm_mask = (batch['bg_mask'] * 2.0 - 1.0).repeat(1, 3, 1, 1)
                        img_size = (256, 256)
                        # norm_premask = (pre_mask * 2.0 - 1.0).repeat(1, 3, 1, 1).float().cpu()
                        ...
                        cat_img  = [
                            ...
                            Resize(img_size)(batch['gt_img']),
                            # Resize(img_size)(norm_mask),
                            Resize(img_size)(x_samples_ddim),
                            # Resize(img_size)(norm_premask)
                            ]
                        log['samples'] = torch.cat(cat_img, dim=-1)
                        images = tensor2images(log)
                        log_local(outdir, images, image_name)
                        # evaluate VAE reconstruction
                        # pred_mask = test_model_kwargs['mask']
                        # x_samples_ddim = model.decode_first_stage(test_model_kwargs['image'])
                        
                        # pred_mask = reverse_mask_tensor(pred_mask)  
                        # pred_img  = reverse_image_tensor(x_samples_ddim)
                        # gt_img = reverse_image_tensor(batch['gt_img'])
                        # bg_img = reverse_image_tensor(batch['bg_img'])
                        # fg_img = reverse_clip_tensor(batch['fg_img'])
                        
                        # ver_border = np.ones((opt.H, 10, 3), dtype=np.uint8) * np.array([0, 0, 250]).reshape((1,1,-1))
                        # for i in range(len(bg_img)):
                        #     image = batch['image_path'][i]
                        #     im_name  = os.path.splitext(os.path.basename(image))[0].split('_')[0]
                        #     cv2.imwrite(os.path.join(result_path, im_name + '.jpg'), pred_img[i])
                        #     cv2.imwrite(os.path.join(result_path, im_name + '_mask.png'), pred_mask[i])
                        #     cat_img = np.concatenate([gt_img[i], ver_border, bg_img[i], ver_border, fg_img[i], ver_border, pred_img[i], ver_border, pred_mask[i]], axis=1)
                        #     cv2.imwrite(os.path.join(grid_path, im_name + '.jpg'), cat_img)
