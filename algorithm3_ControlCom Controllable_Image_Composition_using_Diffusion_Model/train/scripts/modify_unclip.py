...
def convert_unet():
    ...
    new_prefix = 'model.diffusion_model'
    for name in sd_ckpt.keys():
        if new_prefix in name:
            old_name = prefix + name.split(new_prefix)[-1].replace('.','')
            if old_name in ckpt_file['state_dict']:
                ckpt_file['state_dict'][name] = ckpt_file['state_dict'].pop(old_name)

def convert_cond_model():
    prefix = 'cond_stage_model.model'
    new_prefix = 'cond_stage_model.transformer.vision_model'
    old_names = list(ckpt_file['state_dict'].keys())
    for name in old_names:
        if prefix in name:
            new_name = new_prefix + name.split(name)[-1]
...
pretrained_model_path='pretrained_models/image-variation-v1-4.ckpt'
print(f"Loading model from {pretrained_model_path}")
ckpt_file=torch.load(pretrained_model_path,map_location='cpu')
sd_ckpt = torch.load('pretrained_models/sd-v1-4.ckpt', map_location='cpu')['state_dict']
new_model_path = 'pretrained_models/image-variation-modified.ckpt'
config = OmegaConf.load('configs/image_variation.yaml')
model = build_model(config)
convert_unet()
convert_cond_model()
add_input_channels(5)
add_output_channels(1)
