...
class FOSEDataset(data.Dataset):
    def __init__(self, dataset_dir='path-to-data-dir/FOSCom'):
...
def vis_random_augtype(batch):
    ...
    border_color = [0, 0, 0]
    ver_border = np.ones((gt_imgs[0].shape[0], 10, 3), dtype=np.uint8) * np.array(border_color).reshape((1,1,-1))
    img_list = []
    for i in range(len(gt_imgs)):
        ...
        if i > 0:
            hor_border = np.ones((10, cat_img.shape[1], 3), dtype=np.uint8) * np.array(border_color).reshape((1,1,-1))
            img_list.append(hor_border)
        img_list.append(cat_img)
    img_batch = np.concatenate(img_list, axis=0)
    return img_batch
